...
def test_inference():
    ...
    input = httpclient.InferInput("testino_in", input_data.shape, "FP32")
    input.set_data_from_numpy(input_data)

    output = httpclient.InferRequestedOutput("testino_out")

    response = triton_client.infer(MODEL_NAME, inputs=[input], outputs=[output])

    output_data = response.as_numpy("testino_out")
